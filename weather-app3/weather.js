...
function getLocation(query, callback) {
    ...
    request.get({ url: url, json: true },
        function (error, response, body) {
            if (error) {
                callback('Hubo un error con el servicio de ubicación', undefined);
            }
            else if (response.statusCode != 200) {
                callback('Acceso denegado. Token inválido', undefined);
            }
            else if (body.features.length < 1) {
                callback('No se encontro la ubicación especificada', undefined);
            }
            else {
        ...
        if (error) {
            ...
            getWeather(response, function (error, response) {
                ...
                if (error) {
                    ...
                    console.log(output);
                    let res = `${response.summary} Actualmente está a ${response.current_temp} °C. Hay ${response.precipitation}% de posibilidad de lluvia.\n`;
